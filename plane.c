...
void init_plane_hitbox(plane_t *plane)
{
    ...
    sfVector2f plane_pos = sfSprite_getPosition(plane->plane);

    plane->hitbox = sfRectangleShape_create();
    sfRectangleShape_setSize(plane->hitbox, (sfVector2f){20, 20});
    sfRectangleShape_setPosition(plane->hitbox, (sfVector2f){plane_pos.x - 50, plane_pos.y - 50});
    sfRectangleShape_setOutlineThickness(plane->hitbox, 1);
    sfRectangleShape_setOutlineColor(plane->hitbox, grey);
...
void init_planes(sprite_t *sprite, char *path)
{
    ...
    for (int i = 0; i < sprite->nb_plane; i++) {
        ...
        sfSprite_setTexture(sprite->plane[i].plane, sprite->plane_text, sfFalse);
        ...
        sfSprite_setOrigin(sprite->plane[i].plane, (sfVector2f){bounds.width / 2, bounds.height / 2});
...
sfVector2f get_next_pos(plane_t *plane, float time, sfVector2f act_pos)
{
    ...
    direction = (sfVector2f){(plane->arr_pos.x - act_pos.x), (plane->arr_pos.y - act_pos.y)};
...
void change_orientation(sfVector2f act_pos, sfVector2f next_pos, plane_t *plane)
{
...
void move_plane(sprite_t *sprite, sfClock *clock_move, sfClock *clock_delay)
{
    ...
    for (int i = 0; i < sprite->nb_plane; i++) {
        ...
        if (sprite->plane[i].is_dep == 1 && sprite->plane[i].is_arr == 0) {
            ...
            sfRectangleShape_setPosition(sprite->plane[i].hitbox, (sfVector2f){next_pos.x - 10, next_pos.y - 10});
        }
    }
